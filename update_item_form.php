...
<header class="w3-container w3-teal w3-center w3-margin-bottom">
    <h1>Update item <?= $id ?> van lijst <?= $name ?></h1>
    <a href="<?php echo $_SERVER['HTTP_REFERER'] ?>">Back</a>
</header>
<section class="w3-container">
    <form action="update_item.php" method="post">
        <label for="desc" class="w3-text-blue"><b>Beschrijving: </b></label>
        <textarea id="desc" class="w3-input w3-border w3-margin-bottom" name="desc" type="text" required><?= $desc ?></textarea>
        <label for="time" class="w3-text-blue"><b>Tijd (min): </b></label>
        <input type="number" id="time" name="time" class="w3-input w3-border w3-margin-bottom" value="<?= $time ?>" required>
        <label for="status" class="w3-text-blue"><b>Status = <?= $status ?>. (Je moet een status selecteren!)</b></label>
